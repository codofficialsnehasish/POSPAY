<?php
    ...
    if (!function_exists('total_vendor')) {
        function total_vendor()
        {

            $user = Auth::user();
            if ($user && $user->hasRole('Admin')) {
                $count = User::role('Vendor')
                        ->where('admin_id', $user->id)
                        ->count();
                return $count;
            }
    ...
    if (!function_exists('total_variation_option_count')) {
        function total_variation_option_count($productId = null)
        {
            ...
            if ($user && $user->hasRole('Admin')) {
                // Get available product IDs for this admin
                $availableProductIds = Product::where('admin_id', $user->id)
                    ->pluck('id');
    ...
    if (!function_exists('low_stock_variation_option_count')) {
        ...
        {
            // auth()->user()->admin?->is_purchase_enabled
            $query = VendorProductStock::query()
                ->where('stock', '<=', $threshold);

            $user = Auth::user();

            if ($user && $user->hasRole('Vendor')) {
                if(!$user->admin?->is_purchase_enabled){
                    return 0;
                }
                // Get available product IDs for this vendor
                $availableProductIds = VendorProduct::where('vendor_id', $user->id)
    ...
    if (! function_exists('top_selling_products')) {
        ...
        function top_selling_products(int $limit = 5, $user_id = null, $vendor_id = null, $from = null, $to = null)
        {
            ...
            $vendorID = collect(); // empty collection default

            // If user is Vendor → return only his ID
            if ($user && $user->hasRole('Vendor')) {
                $vendorID = collect([$user->id]);
            }

            // If user is Admin → return all Vendor IDs under this Admin
            if ($user && $user->hasRole('Admin')) {
            ...
            // If vendor_id passed manually override
            if (isset($vendor_id)) {
                $vendorID = collect([$vendor_id]);
            }
    ...
    if (!function_exists('low_stock_products')) {
        function low_stock_products(int $limit = 5)
        {
            ...
            // Base stock query from VendorProductStock
            $stockQuery = VendorProductStock::query()
                ->where('stock', '<=', 10) // low stock <=10
                ->with(['variation.product.categories', 'variation.options']);
            ...
            if ($user && $user->hasRole('Vendor')) {
                ...
                // Vendor purchase disabled → No results
                if (!$user->admin?->is_purchase_enabled) {
                    return collect([]);
                }

                // Vendor available product IDs
                $availableProductIds = VendorProduct::where('vendor_id', $user->id)
                    ->where('availability', 1)
                    ->pluck('product_id');

                // Filter stock → vendor's products only
                $stockQuery->whereHas('variation.product', function ($q) use ($availableProductIds) {
                    $q->whereIn('products.id', $availableProductIds);
                });
            }
            ...
            if ($user && $user->hasRole('Admin')) {
                ...
                // All available product IDs of these vendors
                $availableProductIds = VendorProduct::whereIn('vendor_id', $vendorIds)
                    ->where('availability', 1)
                    ->pluck('product_id');

                // Filter stock by these products only
                $stockQuery->whereHas('variation.product', function ($q) use ($availableProductIds) {
                    $q->whereIn('products.id', $availableProductIds);
                });
            }
            ...
            return $stockQuery->get()
                ->map(function ($stock) {
                    ...
                    $product  = $stock->variation->product;
                    $variation = $stock->variation;
                    $option    = $variation->options->first(); // assuming single option
    ...
    if (!function_exists('order_heatmap_data')) {
        function order_heatmap_data($params = ['period' => 'today'], $vendor_id = null)
        {
            $user = Auth::user();
            ...
            // -------- QUERY --------  
            $raw = \App\Models\Order::whereIn('vendor_id', $vendorID)
    ...
    if (!function_exists('category_heatmap_data')) {
        function category_heatmap_data($params = ['period' => 'today'])
        {
            ...
            $data = \App\Models\OrderItems::select(
                ...
                ->whereIn('orders.vendor_id', $vendorId)
            ...
            if (is_array($range) && ($range['period'] ?? null) === 'custom') {
                ...
                foreach ($labels as $i => $label) {
                    ...
                    if ($user && $user->hasRole('Admin')) {
                        ...
                        $querySales->whereIn('vendor_id', $vendorIds);
                        $queryPurchase->whereIN('vendor_id', $vendorIds);
                    }
